...
class siRNA_Encoder(nn.Module):
    def __init__(self, vocab_size, embedding_dim, lstm_dim, n_head, n_layers):
        ...
        self.biLSTM = nn.LSTM(5,lstm_dim,2,bidirectional = True) # 68
...
class Oligo(nn.Module):
    def __init__(self, vocab_size = 26, embedding_dim = 128, lstm_dim = 32,  n_head = 8, n_layers = 1): # 4928
        ...
        self.classifier = nn.Sequential(
            nn.Linear(5120, 256), # 1216 + 3392 + 128 + 128 + 24
            nn.ReLU(),
            nn.Dropout(0.02),
            nn.Linear(256, 64),
            nn.ReLU(),
    ...
    def forward(self, siRNA, mRNA, siRNA_FM, mRNA_FM, td):
        ...
        td = self.flatten(td)
        merge = torch.cat([siRNA,mRNA,siRNA_FM,mRNA_FM],dim = -1) # 4864
        #merge = torch.cat([siRNA,siRNA_FM,td],dim = -1) # 4608
