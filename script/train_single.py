...
def run(Args):
	...
	dataset = pd.read_csv(path + Args.datasets[0] + '_siRNA.csv', dtype=str)
	...
	for i_fold in range(Args.kfold):
		...
		valid_ds = DataLoader(data_process_loader(valid_df.index.values, valid_df.label.values,valid_df.y.values, valid_df, Args.datasets[0]),**params)
		...
		for epoch in range(Args.epoch):
			for i, data in enumerate(train_ds):
				...
				mRNA_FM = data[3].to(device)
				label = data[4].to(device)
				output = OFmodel(siRNA,mRNA,siRNA_FM,mRNA_FM)
