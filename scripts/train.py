...
def val(model, criterion, dataloader):
	...
	for i, data in enumerate(dataloader):
		...
		td = data[6].to(device)
		pred = model(siRNA,mRNA,siRNA_FM,mRNA_FM,td)
...
def run(Args):
	...
	path = '/mnt/inspurfs/user-fs/qhsky1/baiyilan/OligoFormer/data/'
	train_df = pd.read_csv(path + Args.datasets[0] + '.csv', dtype=str)
	# train_df,valid_df = train_test_split(train_df,test_size=0.2,shuffle=True,random_state=42)
	# train_df = train_df.reset_index(drop=True)
	# valid_df = valid_df.reset_index(drop=True)
	valid_df = pd.read_csv(path + Args.datasets[1] + '.csv',  dtype=str)
	test_df = pd.read_csv(path + Args.datasets[1] + '.csv', dtype=str)
	...
	for epoch in range(Args.epoch):
		for i, data in enumerate(train_ds):
			...
			label = data[4].to(device)
			td = data[6].to(device)
			output = OFmodel(siRNA,mRNA,siRNA_FM,mRNA_FM,td)
