...
for FLANK in _FLANK:
    ...
    with open(path + 'flanking/' + str(FLANK) + '/fasta/' + 'Hu_mRNA.fa','a') as f:
        for i in range(Hu.shape[0]):
            f.write('>RNA' + str(i) + '\n')
            f.write(Hu.iloc[i,-1] + '\n')
    Hu_o['mRNA'] = Hu['flanking']
    Hu_o.to_csv(path + 'flanking/' + str(FLANK) + '/Hu.csv',index=False)
    with open(path + 'flanking/' + str(FLANK) + '/fasta/' + 'new_mRNA.fa','a') as f:
        for i in range(new.shape[0]):
            f.write('>RNA' + str(i) + '\n')
            f.write(new.iloc[i,-1] + '\n')
    new_o['mRNA'] = new['flanking']
    new_o.to_csv(path + 'flanking/' + str(FLANK) + '/new.csv',index=False)
    with open(path + 'flanking/' + str(FLANK) + '/fasta/' + 'Taka_mRNA.fa','a') as f:
        for i in range(Taka.shape[0]):
            f.write('>RNA' + str(i) + '\n')
