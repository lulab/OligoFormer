...
class data_process_loader(data.Dataset):
    def __init__(self, df_index, labels, y, df, dataset):
        ...
        self.siRNA_ref = {}
        with open('./data/fasta/' + self.dataset + '_siRNA.fa') as f:
            for line in f:
                if line.startswith('>'):
        ...
        self.mRNA_ref = {}
        with open('./data/fasta/' + self.dataset + '_mRNA.fa') as f:
            for line in f:
                if line.startswith('>'):
                    name = line.strip().replace('>','')
                else:
    ...
    def __getitem__(self, index):
        ...
        # siRNA RNA-FM
        siRNA_seq = self.df.iloc[index]['siRNA']
        siRNA_FM = np.load('./data/RNAFM/' + self.dataset + '_siRNA/representations/'+ str(self.siRNA_ref[siRNA_seq]) + '.npy')
        # mRNA RNA-FM
        mRNA_seq = self.df.iloc[index]['mRNA']
        mRNA_FM = np.load('./data/RNAFM/' + self.dataset + '_mRNA/representations/' + str(self.mRNA_ref[mRNA_seq])+'.npy') 
