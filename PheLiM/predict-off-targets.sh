...
echo ' ' > "$4"_offtarget_predictions.tab
...
awk 'BEGIN{OFS="\t";target="";sirna="";totscore=0.0;} {
		...
		} END{print target,sirna,-totscore;}' temp.txt | sed '1d' >> "$4"_offtarget_predictions.tab

sed -i '1imRNA\tpos\tPheLiM_Score' "$4"_offtarget_predictions.tab
sed -i '2d' "$4"_offtarget_predictions.tab
